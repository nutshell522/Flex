  ...
  return class TWzipcode {
    ...
    constructor(selector = '.twzipcode', options = {}) {
      ...
      if (Helper.isString(selector)) {
        ...
        this.container = selector;
      } else if (Helper.isObject(selector)) {
        //參數1=選項
    ...
    init() {
      ...
      this.container.forEach((elem) => {
        ...
        if (Helper.isDOM(c, d)) {
          ...
          let defaultDistrictOption = `<option value="">${
            'label' in d.dataset ? d.dataset.label : this.options.district.label
          }</option>`;
          ...
          let label =
            'label' in c.dataset ? c.dataset.label : this.options.county.label;
          const had =
            c.dataset.hasOwnProperty('loaded') && Boolean(c.dataset.loaded);
          ...
          Object.keys(this.database).forEach((county) => {
            ...
            const c2 =
              (Helper.isString(hideCounty) || Helper.isArray(hideCounty)) &&
              hideCounty.includes(county); //有指定隱藏的縣市
            if (!(c1 || c2)) {
    ...
    bindEvents() {
      ...
      const onCountyChange = function (e) {
        ...
        if (db) {
          for (let district in db) {
            ...
            const c2 =
              (Helper.isString(hideDistrict) || Helper.isArray(hideDistrict)) &&
              hideDistrict.includes(district); //有指定隱藏的縣市
            if (!(c1 || c2)) {
          ...
          let label =
            'label' in this.district.dataset
              ? this.district.dataset.label
              : districtLabel;
      ...
      const onDistrictChange = function (e) {
        ...
        if (
          database.hasOwnProperty(this.county.value) &&
      ...
      const onZipcodeKeyup = function (e) {
        ...
        loop1: for (let county in database) {
          loop2: for (let district in database[county]) {
            if (
              database[county].hasOwnProperty(district) &&
              self.value == database[county][district]
            ) {
              find = {
                county: county,
                district: district,
      ...
      Object.values(this.instance).forEach((obj) => {
        if (Helper.isDOM(obj.county, obj.district)) {
          const hadCounty =
            obj.county.dataset.hasOwnProperty('listener') &&
            Boolean(obj.county.dataset.listener);
          const hadDistrict =
            obj.district.dataset.hasOwnProperty('listener') &&
            Boolean(obj.district.dataset.listener);
          if (!hadCounty) {
        ...
        if (Helper.isDOM(obj.zipcode)) {
          const hadZipcode =
            obj.zipcode.dataset.hasOwnProperty('listener') &&
            Boolean(obj.zipcode.dataset.listener);
          if (!hadZipcode) {
        ...
        if (Helper.isDOM(obj.address)) {
          const hadAddress =
            obj.address.dataset.hasOwnProperty('listener') &&
            Boolean(obj.address.dataset.listener);
          if (!hadAddress) {
    ...
    isIslands(cond) {
      ...
      const length = Object.keys(instance).length;
      let i =
        Helper.isString(cond) || Helper.isArray(cond)
          ? cond
          : this.options.islands_key;
      let r = {};
    ...
    parseAddress(addr) {
      ...
      const re =
        /(?<zipcode>\d+)?(?<county>\D+[縣市])(?<district>\D+?(市區|鎮區|鎮市|[鄉鎮市區]))(?<village>\D+?[村里])?(?<neighbor>\d+[鄰])?(?<road>\D+?(村路|[路街道段]))?(?<section>.+?段)?(?<lane>.+?巷)?(?<alley>\d+弄)?(?<num>.+號)?(?<others>.+)?/gi;
